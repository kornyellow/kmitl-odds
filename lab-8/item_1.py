...
if __name__ == "__main__":
    ...
    # Put all data in tree
    tree = Tree()
    for e in data:
        node = Node(e, inp.count(e))
        tree.insert(node)

    # Pull all data from tree and put in the list with inorder method
    node_list = tree.inorder(tree.root)
    temp_list = [node_list.pop()]

    # Shuffle node around by Huffman's rules
    ...
    # Traverse in a tree to put encoded value in dict
    # and print encoded dict to std output
    encode_dict = {}
    encode_dict = traverse_and_dict(huffman, encode_dict)
    print(encode_dict)

    # Print Huffman's binary tree
    tree.print(huffman)

    # Encode text from input and print to std output
    print("Encoded! : ", end="")
    for d in inp:
